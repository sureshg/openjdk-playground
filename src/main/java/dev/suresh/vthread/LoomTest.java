...
//      var ff = new ExecutorCompletionService<String>(Executors.newSingleThreadExecutor());
//
//      try (StructuredExecutor test = StructuredExecutor.open("test")) {
//        test.fork(tasks.get(0),
//           (e, f) -> {
//            e.shutdown();
//          })
//      }.stream().map(test::fork);
//        test.submit(tasks).map(Future::resultNow).map(String::length).forEach(out::println);
//      }
//
//      execSvc.submit(tasks).map(Future::resultNow).map(String::length).forEach(out::println);
//
//      end = System.nanoTime();
//      out.println(">>> " + (end - start) / 1_000_000);
//
