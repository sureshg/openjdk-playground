...
public class JavaApp {
    ...
    void main(String[] args) throws Exception {
        ...
        out.printf("%n✧✧✧✧✧ JVM Memory -> Total Allocated : %.2fGB, Free: %.2fGB, Max Configured: %.2fGB,  ✧✧✧✧✧%n", rt.totalMemory() / sizeUnit, rt.freeMemory() / sizeUnit, rt.maxMemory() / sizeUnit);

        out.printf("%n✧✧✧✧✧ Processes ✧✧✧✧✧%n");
        var ps = ProcessHandle.allProcesses().sorted(ProcessHandle::compareTo).toList();
        ps.forEach(p -> {
            var pInfo = STR."\{p.pid()} : \{p.info()}";
            out.println(pInfo);
        });
        ...
        Stream.of("java", "kotlin", "scala", " ").map(String::toUpperCase).filter(s -> !s.isBlank()).mapMulti((s, consumer) -> {
            consumer.accept(s);
        ...
        var stats = """
                ...
                | Process Time   : %-5dms |
                +--------------------------+
                """.formatted(ps.size(), dns.size(), issuers.size(), tz.size(), cs.size(), locales.length, countries.length, languages.length, currencies.size(), env.size(), props.size(), (currTime - vmTime), (start - vmTime), (currTime - start));
        out.println(stats);
    }
    ...
    private void webServer() throws IOException {
        ...
        server.createContext("/", t -> {
            ...
            var res = "Java %s running on %s %s".formatted(System.getProperty("java.version"), System.getProperty("os.name"), System.getProperty("os.arch"));
        ...
        out.printf("Started in %d millis! (JVM: %dms, Server: %dms)%n", (currTime - vmTime), (start - vmTime), (currTime - start));
    }
}
