...
object GithubAction {
  ...
  /**
   * The default environment variables that GitHub sets are available to
   * every step in a workflow. Environment variables are case-sensitive.
   * Environment variables are always interpolated on the virtual
   * machine runner. However, parts of a workflow are processed
   * by GitHub Actions and are not sent to the runner. You cannot
   * use environment variables in these parts of a workflow file.
   * Instead, you can use contexts. For example, an if conditional,
   * which determines whether a job or step is sent to the runner, is
   * always processed by GitHub Actions. You can use a context in an
   * if conditional statement to access the value of an environment
  ...
  object Env {
     ...
     * GitHub removes special characters, and uses the name __run when
     * the current step runs a script without an id. If you use the same
     * script or action more than once in the same job, the name will
     * include a suffix that consists of the sequence number preceded by
     * an underscore. For example, the first script you run will have
    ...
    /**
     * The path where an action is located. This property is only
     * supported in composite actions. You can use this path to access
     * files located in the same repository as the action. For example,
    ...
    /**
     * The name of the base ref or target branch of the pull request in
     * a workflow run. This is only set when the event that triggers a
     ...
     * in a repository. This number begins at 1 for the workflow run's
     * first attempt, and increments with each re-run. For example, 3.
     */
     ...
     * repository. This number begins at 1 for the workflow's first run,
     * and increments with each new run. This number does not change if
     * you re-run the workflow run. For example, 3.
     */
    ...
    /**
     * The name of the workflow. For example, My test workflow. If
     * the workflow file doesn't specify a name, the value of this
     * variable is the full path of the workflow file in the repository.
     */
    ...
    /**
     * The path to a temporary directory on the runner. This directory
     * is emptied at the beginning and end of each job. Note that files
     * will not be removed if the runner's user account does not have
     ...
     * GitHub-hosted runners. For more information, see "About
